...
public class HttpRequest : HttpBase
{
    ...
    internal void BuildHeader<TWriter>(ref TWriter writer) where TWriter : IByteBlockWriter
    {
        writer.WriteNormalString(this.Method.ToString(), Encoding.UTF8);//Get
        ...
        if (this.m_query.Count > 0)
        {
            ...
            {
                writer.WriteNormalString(item, Encoding.UTF8);
                TouchSocketHttpUtility.AppendEqual(ref writer);
                var value = this.m_query[item];
                if (value.HasValue())
                {
                    writer.WriteNormalString(Uri.EscapeDataString(value), Encoding.UTF8);
                }
        ...
        writer.WriteNormalString(this.ProtocolVersion, Encoding.UTF8);//1.1
        TouchSocketHttpUtility.AppendRn(ref writer);//换行

        foreach (var headerKey in this.Headers.Keys)
        {
            writer.WriteNormalString(headerKey, Encoding.UTF8);//key
            TouchSocketHttpUtility.AppendColon(ref writer);//冒号
            TouchSocketHttpUtility.AppendSpace(ref writer);//空格
            writer.WriteNormalString(this.Headers[headerKey], Encoding.UTF8);//value
            TouchSocketHttpUtility.AppendRn(ref writer);//换行
        }
