...
public interface IDmtpActor : IDisposableObject, IOnlineClient, IClosableClient, IIdClient
{
    ...
    Task SendPackageAsync(ushort protocol, IPackage package, CancellationToken token = default);
    ...
    Task SendStringAsync(ushort protocol, string value, CancellationToken token = default);
