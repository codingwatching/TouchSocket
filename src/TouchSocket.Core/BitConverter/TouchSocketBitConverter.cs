...
public sealed class TouchSocketBitConverter
{
           ...
           where TTarget : unmanaged
    {
        ...
        using (var writer = new ByteBlockWriter(bytesToConvert))
        {
            foreach (var item in sourceSpan)
            {
                writer.WriteValue(item, sourceEndianType);
            }

            var reader = new BytesReader(writer.Span);

            for (var i = 0; i < length; i++)
            {
                targetSpan[i] = reader.ReadValue<TTarget>(targetEndianType);
            }
        }
        ...
        {
            // 端序一致，使用MemoryMarshal.Cast进行高效转换
            var sourceBytesSpan = MemoryMarshal.AsBytes(sourceSpan);
            var targetBytesSpan = MemoryMarshal.AsBytes(targetSpan);

            sourceBytesSpan.Slice(0, bytesToConvert).CopyTo(targetBytesSpan);
        }
        else
        {
            using (var writer = new ByteBlockWriter(bytesToConvert))
