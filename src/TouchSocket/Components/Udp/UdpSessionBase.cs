    ...
    protected async Task ProtectedSendAsync(EndPoint endPoint, IList<ArraySegment<byte>> transferBytes)
    {
        ...
        if (this.m_dataHandlingAdapter == null || !this.m_dataHandlingAdapter.CanSplicingSend)
        {
            ...
            using (var byteBlock = new ByteBlock(length))
            {
                ...
                if (this.m_dataHandlingAdapter == null)
                {
                    await this.ProtectedDefaultSendAsync(endPoint, byteBlock.Memory, CancellationToken.None).ConfigureAwait(EasyTask.ContinueOnCapturedContext);
                }
                else
                {
                    await this.m_dataHandlingAdapter.SendInputAsync(endPoint, byteBlock.Memory,CancellationToken.None).ConfigureAwait(EasyTask.ContinueOnCapturedContext);
                }
            }
        }
        else
